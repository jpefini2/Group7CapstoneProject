...
namespace AdvisementManagerWebApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login([Bind] LoginViewModel model, int? id)
        {
            ...
            if (!(String.IsNullOrEmpty(username) || String.IsNullOrEmpty(password)))
            {
                ...
                else
                {
                    ...
                    return RedirectToRoute(new { action = "Students", controller = "Student"});
                }
            }
            return View(model);
        }
        ...
        {
            var username = Request.Cookies["AdminManager.LoginUser"];
            if (loginDAL.GetUserType(username) == LoginType.ADMIN)
            {
                return RedirectToRoute(new { action = "Students", controller = "Student"});
            }
            else
        ...
        {
            Response.Cookies.Delete("AdminManager.LoginUser");
            Response.Cookies.Delete("AdminManager.LoginSession");
            TempData.Clear();
        }
